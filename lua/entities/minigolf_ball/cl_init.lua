...
net.Receive("Minigolf.GetBallForce", function()
	...
	LocalPlayer()._InputtingForceBall = ball
...
net.Receive("Minigolf.GetBallForceCancel", function()
	...
	LocalPlayer()._InputtingForceBall = false
	currentForce = 0
	drawingName = nil
end)
...
hook.Add("Think", "Minigolf.AdjustPowerAutomaticallyOnAutoMode", function(cmd, x, y, ang)
	if (not IsValid(LocalPlayer()._InputtingForceBall)) then
		return
	end
...
hook.Add("InputMouseApply", "Minigolf.AdjustPowerWithButtonsAndScrolla", function(cmd, x, y, ang)
	...
	if (not IsValid(LocalPlayer()._InputtingForceBall) or not IsValid(activeWeapon) or activeWeapon:GetClass() ~= "minigolf_club") then
		return
	end
	...
	local start = LocalPlayer()._InputtingForceBall:GetStart()
...
hook.Add("PostDrawTranslucentRenderables", "Minigolf.DrawDirectionArea", function(isDrawingDepth, isDrawSkybox)
	...
	if (IsValid(LocalPlayer()) and LocalPlayer()._InputtingForceBall) then
		...
		if (IsValid(ball)) then
			...
			surface.DrawTexturedRectUV(-(texWidth * .5), FORCE_MAX * -currentForce, 256, forceHeight, 0, 1 - currentForce,
				1, 1)
				...
				if (IsValid(ball)) then
					...
					if (IsValid(player)) then
						if (player == LocalPlayer()) then
							...
							if (LocalPlayer()._InputtingForceBall) then
								if (showHints) then
					...
					for _, text in ipairs(texts) do
						...
						draw.SimpleTextOutlined(text, "MinigolfHuge", -PADDING_FLOOR_TEXT, startY, Minigolf.COLOR_LIGHT,
							TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP, 5, Minigolf.COLOR_DARK)
					...
					for _, text in ipairs(textsRight) do
						...
						draw.SimpleTextOutlined(text, "MinigolfHuge", PADDING_FLOOR_TEXT, startY,
							Minigolf.COLOR_SECONDARY,
							TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 5, Minigolf.COLOR_DARK)
...
hook.Add("KeyRelease", "Minigolf.CheckForHit", function(player, key)
	if (key == IN_ATTACK and not drawingName and LocalPlayer()._InputtingForceBall) then
		LocalPlayer()._InputtingForceBall = false
...
hook.Add("Think", "Minigolf.CancelIfNotNearBall", function()
	...
	if (IsValid(ball)) then
		if (LocalPlayer()._InputtingForceBall == ball and not LocalPlayer():IsInDistanceOf(ball, DISTANCE_TO_BALL_MAX)) then
			LocalPlayer()._InputtingForceBall = false
			lastPersonalForce = currentForce
			LocalPlayer():SetBallGivingForce(nil)
			...
			net.SendToServer()
		elseif (not LocalPlayer()._InputtingForceBall and UnPredictedCurTime() - lastBallInteraction > 1) then
