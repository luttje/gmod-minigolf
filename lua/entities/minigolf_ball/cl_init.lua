        ...
        if (IsValid(ball)) then
          ...
          if (IsValid(player)) then
            if (player == LocalPlayer()) then
              if (showHints) then
                texts[#texts + 1] = string.format("Stuck? Press '%s' + '%s' on the ball to move back to the start",
          ...
          for _, text in ipairs(textsRight) do
            ...
            draw.SimpleTextOutlined(text, "MinigolfHuge", PADDING_FLOOR_TEXT, startY, Minigolf.COLOR_SECONDARY,
