...
hook.Add("PostDrawTranslucentRenderables", "Minigolf.EntityFlag.DrawFlags", function(isDrawingDepth, isDrawingSkybox)
	...
	for _, flags in pairs(flagEntities) do
		...
		if (IsValid(hole)) then
			...
			-- TODO: Allow changing other aspects of the flag
			local adjustedColor = hook.Call("Minigolf.GetAdjustedHoleFlagColor", Minigolf.GM(), hole, color)

			if (adjustedColor ~= nil) then
				color = adjustedColor
			end

			local modelBoundsMin, modelBoundsMax = flags:GetModelBounds()
			local flagOrigin = flags:GetPos() - Angle(0, 1, 0):Forward() + (Angle(0, 1, 0):Right() * 1.5) +
				modelBoundsMax
