...
hook.Add("Minigolf.PlayerFinishedHole", "MyAddon.RewardTokensToPlayer", function(player, ball, start, strokes)
	...
	if (reward > 0) then
		Minigolf.Messages.Send(player,
			string.format("You got rewarded %d %s for your %s", reward, PS.Config.PointsName, named), nil,
			Minigolf.TEXT_EFFECT_CASH)

		-- For PointShop 1:
		-- player:PS_GivePoints(reward)

		-- For PointShop 2:
		player:PS2_AddStandardPoints(reward)

		-- For DarkRP:
		-- player:addMoney(reward)

		-- For Clockwork Schemas:
		-- Clockwork.player:GiveCash(player, reward, named, true)
	end
end)
