    ...
    local totalHeight = 0
		...
		for i, msgData in ipairs(allMessages)do
			if(msgData.VanishTime and msgData.VanishTime < UnPredictedCurTime())then
				...
				local textX, textY = (scrW * .5) - (width * .5), PADDING + totalHeight
				local widthIcon, heightIcon

				if(fontIcon == nil or msgData.Icon == "NONE" or not msgData.Icon)then          
          local x = textX - HALF_PADDING
          local y = textY - HALF_PADDING
          local fullWidth = width + PADDING
          local fullHeight = height + PADDING
          ...
          local y = textY - HALF_PADDING
          local fullWidth = width + PADDING
          local fullHeight = height + PADDING + heightIcon + PADDING
          ...
          surface.DrawRect(x, y, fullWidth, fullHeight)
					...
					textY = textY + PADDING + heightIcon
				end

				-- Increment the total height with the current message height
				totalHeight = textY + height + HALF_PADDING
				...
				if(msgData.TextEffect == TEXT_EFFECT_SPARKLE)then
