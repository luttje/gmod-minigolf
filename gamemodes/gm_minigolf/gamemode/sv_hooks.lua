...
hook.Add("Minigolf.GetGoalMessage", "Minigolf.CustomGoalMessagesForCertainStrokes",
  function(player, goal, strokes, start)
    local receivers = Minigolf.Teams.GetOtherPlayersOnTeam(player)
    local par = start:GetPar()

    if (strokes == 0) then
      Minigolf.Messages.Send(receivers,
        player:Nick() .. " got an impossible run(0 strokes) at '" .. start:GetHoleName() .. "'", "¡",
        Minigolf.TEXT_EFFECT_SPARKLE)
      player:PlaySound("vo/ravenholm/madlaugh04.wav")
      return
    elseif (strokes == 1) then
      Minigolf.Messages.Send(playerLibrary.GetAll(),
        player:Nick() .. " got a HOLE IN ONE on '" .. start:GetHoleName() .. "'", "@", Minigolf.TEXT_EFFECT_SPARKLE)
      player:PlaySound("vo/k_lab/kl_excellent.wav")
      return
    elseif (strokes >= start:GetMaxStrokes()) then
      Minigolf.Messages.Send(receivers,
        player:Nick() ..
        " struggled to get to the end of '" ..
        start:GetHoleName() .. "' with " .. strokes .. " " .. Minigolf.Text.Pluralize("stroke", strokes) .. "!", "~")
...
hook.Add("Minigolf.TimeLimitReached", "Minigolf.OutOfTimeMessage", function(player, ball, start)
  Minigolf.Messages.Send(Minigolf.Teams.GetOtherPlayersOnTeam(player),
    "Disqualified! " .. player:Nick() .. " ran out of time on '" .. start:GetHoleName() .. "'", nil,
...
hook.Add("Minigolf.StrokeLimitReached", "Minigolf.StrokeLimitMessage", function(player, ball, start)
  Minigolf.Messages.Send(Minigolf.Teams.GetOtherPlayersOnTeam(player),
    "Disqualified! " .. player:Nick() .. " has " .. start:GetMaxStrokes() ..
    " strokes on '" .. start:GetHoleName() .. "'", "¢", Minigolf.TEXT_EFFECT_ATTENTION)
end)
...
hook.Add("Minigolf.BallOutOfBounds", "Minigolf.OutOfBoundsMessage", function(player, ball, bound)
  ...
  Minigolf.Messages.Send(Minigolf.Teams.GetOtherPlayersOnTeam(player),
    player:Nick() .. " went out of bounds at '" .. start:GetHoleName() .. "'", "Ò")
end)

hook.Add("Minigolf.PlayerGivesUp", "Minigolf.PlayerGivesUpMessage", function(player, start)
  Minigolf.Messages.Send(Minigolf.Teams.GetOtherPlayersOnTeam(player), player:Nick() .. " gave up!", nil,
