hook.Add("Minigolf.CanStartPlaying", "Minigolf.PreventSpectatorsToPlay", function(player, start)
	...
	if (player:Team() == TEAM_MINIGOLF_SPECTATORS) then
		Minigolf.Messages.Send(player, "Spectators can't play, press 'T' to create or join a team!", nil,
...
hook.Add("Minigolf.CanStartPlaying", "Minigolf.RestrictWhenTeamsPlaying", function(player, start)
	for _, otherPly in pairs(team.GetPlayers(player:Team())) do
		...
		if (activeHole) then
			if (activeHole == start) then
				...
				Minigolf.Messages.Send(player,
					string.format("Another one of your team members is already playing on hole '%s'",
						activeHole:GetHoleName()), nil, Minigolf.TEXT_EFFECT_DANGER)
			end
		...
		if (IsValid(swappingHole) and swappingHole ~= start) then
			Minigolf.Messages.Send(player,
				string.format("Your team member is waiting for someone to swap in on hole '%s'",
					swappingHole:GetHoleName()), nil, Minigolf.TEXT_EFFECT_DANGER)
