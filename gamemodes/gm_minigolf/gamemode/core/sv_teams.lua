...
concommand.Add("team_join", function(player, cmd, args)
	...
	Minigolf.Messages.Send(team.GetPlayers(player:Team()), player:Nick() .. " joined team '" .. targetTeam.Name .. "'",
		"N")
end)
...
concommand.Add("team_leave", function(player, cmd, args)
	...
	if (IsValid(player:GetActiveHole())) then
		Minigolf.Messages.Send(player, "Can not leave the team whilst your playing a hole!", nil,
...
concommand.Add("team_kick", function(player, cmd, args)
	...
	if (IsValid(target:GetActiveHole())) then
		Minigolf.Messages.Send(player, "You cannot kick this player whilst they're playing a hole!", nil,
			Minigolf.TEXT_EFFECT_ATTENTION)
		return
	end
...
concommand.Add("team_set_rank", function(player, cmd, args)
	...
	if (not player:GetTeamLeader()) then
		Minigolf.Messages.Send(player, "You aren't team leader, so can't demote or promote players!", nil,
			Minigolf.TEXT_EFFECT_ATTENTION)
		return
	end
	...
	if (isNowTeamLeader) then
		...
		Minigolf.Messages.Send(team.GetPlayers(teamID), player:Nick() .. " was promoted to team leader", "\\",
		...
		Minigolf.Messages.Send(team.GetPlayers(teamID), player:Nick() .. " was demoted to regular team member", "\\",
			Minigolf.TEXT_EFFECT_ATTENTION)
	end
end)
...
net.Receive("Minigolf.TryUpdateTeam", function(len, player)
	...
	if (oldName ~= teamName) then
		Minigolf.Messages.Send(team.GetPlayers(player:Team()),
			player:Nick() .. " updated team '" .. oldName .. "' to become '" .. teamName .. "'", "N")
	else
	...
	if (not Minigolf.Teams.All[player:Team()]) then
		...
		if (not player._HasInitializedInOwnTeam) then
			...
			Minigolf.Messages.Send(player,
				string.format("A team named '%s' has been created for you. It's password is '%s'", teamName,
					randomPassword), "N")
		end
	end
end)
