...
function Minigolf.Messages.Print(msg, icon, textEffect)
	...
	if(textEffect == TEXT_EFFECT_ATTENTION)then
		color = colorChat
	elseif(textEffect == TEXT_EFFECT_DANGER)then
		color = colorNegative
	elseif(textEffect == TEXT_EFFECT_SPARKLE)then
		color = Color(255, 195, 18)
	elseif(textEffect == TEXT_EFFECT_CASH)then
		color = colorPositive
	end
