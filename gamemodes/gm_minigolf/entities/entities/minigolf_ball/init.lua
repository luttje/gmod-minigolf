...
function ENT:OnRemove()
	hook.Call("MinigolfBallRemove", gm(), self:GetPlayer(), self);
end
...
function ENT:Think()
	...
	if(velocityLength < 5)then
		...
		if(not self:GetStationary())then
			...
			hook.Call("MinigolfBallRolledStationary", gm(), self)
		end
	else
...
function ENT:ShowForceMeter(shouldShow)
	...
	if(shouldShow)then
		...
		-- Delay this until we know the ball has been created clientside
			teamPlayer:OnEntityExists(self, function(teamPlayer, entity)
...
function ENT:SetPlayer(player)
	...
	if(not self.hasInitialized)then
		...
		hook.Call("MinigolfBallInit", gm(), player, self);
	end
end
...
net.Receive("Minigolf.SetBallForce", function(len, ply)
	...
	if(IsValid(ball))then
		...
		if(givenForce == -1)then
			-- Requested to cancel the ball use.
		...
		local hookCall = hook.Call("MinigolfPlayerHitBall", gm(), ply, ball)
